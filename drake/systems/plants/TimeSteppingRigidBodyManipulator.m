...
%         B = zeros(size(x0));
    ...
    function [xdn,dxdot] = updateConvex(obj,h,x,u,w)
...
%       end
      ...
      %you're going to actually use the h that is passed in for trajectory
      %optimization so you commented out this line.  
      %h = obj.timestep;
      ...
      [vToqdot,dVqInv] = obj.manip.vToqdot(kinsol);
      VqInv = vToqdot;
      ...
      if isempty(active)
          ...
          if (obj.num_u>0)
              ...
              dvdot = [zeros(num_q,1),...
                  -Hinv*matGradMult(dH(:,1:num_q),vdot) + Hinv*dtau(:,1:num_q),...
                  +Hinv*dtau(:,1+num_q:end), Hinv*B];
          else
              vdot = -Hinv*C;
              dvdot = [zeros(num_q,1),...
                  Hinv*(-matGradMult(dH(:,1:num_q),vdot) - dC(:,1:num_q)),...
                  Hinv*(-dC(:,num_q+1:end))];
          end
          
          dxdot = [...
              zeros(num_q,1), matGradMult(dVqInv, v), VqInv, zeros(num_q,num_u);
              dvdot];
          %end modified code
          ...
          %dxdot = zeros(num_q,1+num_q+num_u);
          ...
          if (obj.num_u>0)
              vdot = Hinv*((B*u-C)*h + vToqdot'*J'*f)/h;
              dtau = matGradMult(dB,u) - dC;
              dvdot = [zeros(num_q,1),...
                  -Hinv*matGradMult(dH(:,1:num_q),vdot) + Hinv*dtau(:,1:num_q),...
                  +Hinv*dtau(:,1+num_q:end), Hinv*B];
